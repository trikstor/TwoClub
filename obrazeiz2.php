...
require_once 'login.php';
  $conn = new mysqli($host, $user, $pass, $data);
   $query  = "SELECT * FROM gdz WHERE otvet='$id'";
...
$delete=$_POST['delete'];
$query  = "DELETE FROM gdz WHERE otvet='$delete'";
 $result = $conn->query($query);
}
   $query  = "SELECT * FROM topics WHERE gdz='$id'";
  $result = $conn->query($query);
  if (!$result) die ("Ошибка системы");
  ...
  for ($j = 0 ; $j < $rows ; ++$j)
  {
    $result->data_seek($j);
    $row = $result->fetch_array(MYSQLI_NUM);
	echo"<a id='dd' href='".$row[1].".php'><div class='cob'>".$row[8]." | ".$row[7]." </a></div><br/>";
  }
?>
