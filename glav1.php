...
<META name="keywords" lang="ru" content="Клуб Двоечников, статьи, ГДЗ, Решебник, найдите, ЕГЭ, ГИА, главная">
<title>Все Статьи | Клуб Двоечников</title>
...
<div align="center" style="background:#999;">
  <h2 style="color:#FFF;">Все Статьи </h2>
<a href="post1.php" class="buttona black rectangular">Написать статью</a>
</div>
<div align="center">
<form action="glav1.php" method="post">
...
<button value="Другое" name="predmet" onclick="javascript: document.forms[...].submit()">Другое</button><br/>
<label for=fader>Выберите класс</label>
(<label for="we">Не выбирать класс</label><input type="checkbox" name="we" id="we" value="1"/>)
<input type="range" min=5 max=11 value=5 id="fader" step=1 name="fader" onchange="outputUpdate(value)">
...
echo '<p><b>Задача | Описание </b></p>';
$result = mysql_query('select * from stat ORDER BY ids DESC limit '.$start_pos.', '.$perpage) or die('error!');
while ($row = mysql_fetch_array($result)) {
if($_POST['fader']==$row[3] || $_POST['we']='404'){
if($row[4]==$_POST['predmet']){
echo "<div class='cob'><a href='".$row[2].".php'>".$row[0]." | ".$row[5]." | ".$row[6]."</a></div></br>";
}	
}
...
echo "<a href='".$row[2].".php'><div class='cob'><b>".$row[5]."</b><br/>".$row[6]." ".$rower."</div></a></br>";
}
	}
?>
</div>
