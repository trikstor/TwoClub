...
require_once 'login.php';
  $conn = new mysqli($host, $user, $pass, $data);
   $query  = "SELECT * FROM studybook WHERE otvet='$id'";
...
<title>Главная | Клуб Двоечников</title>
<link rel="icon" href="https://twoclub.ru/favicon.ico" type="image/x-icon">
...
<b id="total" style="vertical-align:central; margin-left:3%; font-size:3em;"><? echo $title ?></b>
</div>
<img src="<? echo $image ?>" width="200" style="margin-top:2%;" /><span style="margin-left:1%; font-size:18px;"><? echo $desc ?></span>
...
<button value="'.$id.'" name="delete" onclick="javascript: document.forms[...].submit()">Удалить</button>
</form>';
}
?>
<br/>
...
$delete=$_POST['delete'];
$query  = "DELETE FROM studybook WHERE otvet='$delete'";
 $result = $conn->query($query);
}
   $query  = "SELECT * FROM stat WHERE gdz='$id'";
  $result = $conn->query($query);
  if (!$result) die ("Ошибка системы");
  ...
  for ($j = 0 ; $j < $rows ; ++$j)
  {
    $result->data_seek($j);
    $row = $result->fetch_array(MYSQLI_NUM);
	echo"<a id='dd' href='".$row[2].".php'><div class='cob'>".$row[6]." | ".$row[5]." </a><b id='ghg'>↑ ↓</b></div><br/>";
  }
?>
